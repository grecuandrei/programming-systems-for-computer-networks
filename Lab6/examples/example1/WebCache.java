...
public class WebCache {
	...
	public String requestURL(String url) {
		...
		if (cache.containsKey(url)) { // daca deja avem in cache continutul 
			...
			entry.incFreq();
			return "ACCESSED: " +  entry.getURL() + " accesed for " +entry.getFrequency() + " times" ; // atunci returnam valoarea din memorie
		}
			...
			while (sizeOnDisk > maxSizeOnDisk) { // cat timp am depasit mai stergem intrari
				...
				for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
					...
					if (wce == null) wce = e;
						...
						if (wce.getFrequency() > e.getFrequency()) wce = e; // alegem pagina cea mai veche accesata pentru stergere (LFU)
					}
				}
				if (wce == null) break;
				// stergem respectiva intrare din cache
				System.out.println("REMOVED because space: " + wce.getURL() + " from cache with freq:" + wce.getFrequency());
		...
		return "ACCESSED: " + entry.getURL()  + " got accesed for " + entry.getFrequency() + " times";
	}
	...
	private void purgeOldPages() {
		...
		for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
			...
			if (diff > maxTimeToKeepPage) {
				...
				System.out.println("REMOVED because time: " + entry.getURL() + " from cache with freq:" + entry.getFrequency());
			}
		}
